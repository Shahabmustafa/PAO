/// Paths to the app's SVG icon assets, used with the [AppIcon] widget
/// instead of Flutter's built-in Material `Icons`.
class AppIcons {
  AppIcons._();

  static const String _base = 'assets/icons';

  static const String wallet = '$_base/wallet.svg';
  static const String add = '$_base/add.svg';
  static const String addCircleOutline = '$_base/add_circle_outline.svg';
  static const String addCircleFilled = '$_base/add_circle_filled.svg';
  static const String camera = '$_base/camera.svg';
  static const String checkCircle = '$_base/check_circle.svg';
  static const String chevronRight = '$_base/chevron_right.svg';
  static const String close = '$_base/close.svg';
  static const String darkMode = '$_base/dark_mode.svg';
  static const String lightMode = '$_base/light_mode.svg';
  static const String delete = '$_base/delete.svg';
  static const String description = '$_base/description.svg';
  static const String edit = '$_base/edit.svg';
  static const String favoriteOutline = '$_base/favorite_outline.svg';
  static const String favoriteFilled = '$_base/favorite_filled.svg';
  static const String filter = '$_base/filter.svg';
  static const String help = '$_base/help.svg';
  static const String homeOutline = '$_base/home_outline.svg';
  static const String homeFilled = '$_base/home_filled.svg';
  static const String inbox = '$_base/inbox.svg';
  static const String info = '$_base/info.svg';
  static const String language = '$_base/language.svg';
  static const String locationPin = '$_base/location_pin.svg';
  static const String lock = '$_base/lock.svg';
  static const String lockReset = '$_base/lock_reset.svg';
  static const String logout = '$_base/logout.svg';
  static const String notifications = '$_base/notifications.svg';
  static const String person = '$_base/person.svg';
  static const String privacyTip = '$_base/privacy_tip.svg';
  static const String radioChecked = '$_base/radio_checked.svg';
  static const String radioUnchecked = '$_base/radio_unchecked.svg';
  static const String search = '$_base/search.svg';
  static const String settingsOutline = '$_base/settings_outline.svg';
  static const String settingsFilled = '$_base/settings_filled.svg';
  static const String settingsSuggest = '$_base/settings_suggest.svg';
  static const String star = '$_base/star.svg';
  static const String visibility = '$_base/visibility.svg';
  static const String visibilityOff = '$_base/visibility_off.svg';
}
